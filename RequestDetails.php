...
$dotenv->load();
if (isset($_GET['details']) && $_GET['details'] == True && isset($_GET['recipe']) && isset($_GET['type'])) {
   $host = $_ENV['PG_HOST'];
   ...
   if ($_GET['type'] == 'recipes') {
      ...
      $request = pg_query(
         $connection,
         "SELECT name, item, price, calories_per_100g, weight
                FROM ingredients WHERE item='$recipe'"
      );
   }
   if (!$request) {
